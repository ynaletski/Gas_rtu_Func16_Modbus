...
void main (void)
{
  ...
  for (;;)
  {
    ...
    { /*обработка ответа Modbus, подключенного к СОМ3*/
      if (Modbus.mode==1) 
        ModbusSlave(Port[2].buf,Port[2].index);
      else
...
void ViewError ()
{
   //nm const unsigned char str[3]={0x4f,0x5,0x5};
 ...
 M: if (err[ind_err] < 10)
	...
	WriteEvent(buf,2);/*запись об установке нештатной ситуации*/
      ...
      //nm for (i=0;i<3;i++) Show5DigitLedSeg(i+1,str[i]);
      i=(ind_err+1)/10;
      //nm Show5DigitLed(4,i);
      i=ind_err+1-i*10;
      //nm Show5DigitLed(5,i);ind_err++;
    }
}
	   ...
	   unsigned char data[],unsigned ind_func,unsigned char num)
{
  ...
  //20.07.2021 YN was: unsigned res; now:
  unsigned res, value_dt;
  //unsigned char flg_date=0, flg_time=0;
  
  float value,lo_scale,hi_scale; mb_err=0; flg_time=flg_date=flg_wr=0;
  ...
  for (i=0;i<*cnt_reg;i++)
  {
    ...
    else
    {
      ...
      //20.07.2021 YN /*функция с номером 16 Время и дата*/
...
void ModbusSlave (unsigned char buf[], unsigned char index)
{
  ...
  if (Modbus.protocol != 1) goto M; 
  mb_err=0;
  for (i=0;i<index;i++) data[i]=buf[i]; 
  addr=data[0];
  if (addr != Device.addr) goto M; 
  flag=0; 
  func=data[1];
  sum_lo=data[index-2]; 
  sum_hi=data[index-1]; 
  CalcCRC16(data,index-2,cr);
  ...
  switch (func)
  {
    ...
    case 16:
            ind_func=7;
            if (data[6]>0) DataToModbus(reg,&count,data,ind_func,7);
            count=6;
    break;
    case 6:
